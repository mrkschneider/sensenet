...
import com.tcb.sensenet.internal.analysis.correlation.mutualInformation.pointwise.NormalizedPMI;
...
public class NormalizedPMITest extends PMITest {
	...
	public void testCalculate() {
		...
		PointwiseMutualInformationStrategy inf = new NormalizedPMI();
		
		assertEquals(-1/-log(p00),inf.calculate(0, 0, freqA,freqB,freqAB),0.01);
		assertEquals(0.22/-log(p01),inf.calculate(0, 1, freqA,freqB,freqAB),0.01);
		assertEquals(1.58/-log(p10),inf.calculate(1, 0, freqA,freqB,freqAB),0.01);
		assertEquals(-1.58/-log(p11),inf.calculate(1, 1, freqA,freqB,freqAB),0.01);
	}
